...
Программа переводит количество минут в формат `Ч:ММ`.

**Как работает код:** целочисленное деление `min//60` даёт часы, остаток от деления `min%60` — минуты. Формат `:02d` дополняет минуты ведущим нулём, чтобы вывод всегда был двузначным.
